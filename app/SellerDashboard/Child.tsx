    ...
    <>
          ...
          products.map((p) => (
            ...
            >
              <div className="relative mb-4 aspect-square overflow-hidden rounded-2xl">
                ...
                />
                {p.category && (
                  <span className="absolute bottom-3 left-3 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-gray-700 shadow">
                    {p.category.name}
                  </span>
                )}
      ...
      {totalPages > 1 && (
        <div className="mt-12 flex justify-center gap-4">
          <Link
            href={`/SellerDashboard?category=${category}&page=${Math.max(1, page - 1)}`}
          ...
          <Link
            href={`/SellerDashboard?category=${category}&page=${Math.min(totalPages, page + 1)}`}
